...
tail by the modified-Liu chi-square moment-matching approximation.
...
Unlike the variance-scaled variant, the weight $`\phi`$ here equalizes
the two components’ contributed null *standard deviations*:

``` math
\phi = \frac{\sqrt{V_\tau}}{\sqrt{V_\mu} + \sqrt{V_\tau}}, \quad
  V_\mu = 2\,s^2, \quad V_\tau = 2\,\mathrm{tr}(M_\tau\Sigma M_\tau\Sigma),
```

where $`V_\mu`$ and $`V_\tau`$ are the null variances of $`(a'z)^2`$ and
$`z'M_\tau z`$. This places relatively more weight on the heterogeneity
component than the variance-scaled rule. Under the null $`T`$ is a
weighted sum of independent $`\chi^2_1`$ variables,
$`T \sim \sum_j \lambda_j \chi^2_{1,j}`$, with weights $`\lambda_j`$ the
eigenvalues of $`\Sigma^{1/2} L_\phi \Sigma^{1/2}`$; the reported
p-value is the upper tail of this mixture.

**Tail method (modified Liu).** The tail is computed by the modified-Liu
approximation, which matches all four cumulants (including kurtosis) of
$`T`$ to a non-central chi-square. It is as fast as the plain Liu method
and typically more accurate, though still an approximation; the Davies
variant remains the reference for extreme tails.

## What This Method Advances

The modified-Liu variant of MixSD uses the package’s modified
moment-matching tail for the standard-deviation-weighted blend.
...
concordant shared effects while the $`M_\tau`$ part captures discordant,
antagonistic, or sparse effects. Compared with the variance-scaled
mixture this rule favors the heterogeneity direction somewhat more,
trading a little power under perfect concordance for robustness under
discordance. Validity requires $`\Sigma`$ to be a correct, full-rank
trait correlation matrix and the per-trait Z-scores to be approximately
Gaussian under the null.

## When This Method Is a Good Fit

Good when the modified-Liu approximation to the MixSD blend is desired.
...
pleio <- run_mixsd_liumod(pleio)
head(pleio@results$mixsd_liumod)
```

## Implementation Files

- R wrapper: `R/run_mixsd_liumod.R`
- C++ kernel or dispatcher: `src/mixsd.cpp`
