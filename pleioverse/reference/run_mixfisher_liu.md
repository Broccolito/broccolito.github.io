...
the `pleio` object with `results$mixfisher_liu` populated (one p-value
per variant).
...
**Tail method (Liu).** The heterogeneity tail \\p\_\tau\\ is computed by
the Liu (Liu-Tang-Zhang) approximation, which matches the first three
cumulants of \\U\_\tau\\ to a single (possibly non-central) chi-square.
